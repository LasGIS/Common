 ...
 * @author MKitchenko
 */
public class XlsFileMaker extends ExcelFileMaker implements FileMaker {

    @Override
    public byte[] makeFileWithErrors(InputStream inputStream, Map<Integer, List<ValidationError>> errors) throws Exception {
        return super.makeFileWithErrors(new HSSFWorkbook(inputStream), errors);
    }
    ...
    public byte[] makeFileWithCredentials(InputStream inputStream, Map<Integer, BatchRegistrationResponseData> credentials) throws Exception {
        ...
        final HSSFFont tableHeaderFont = workbook.getSheetAt(0).getRow(0).getCell(0).getCellStyle().getFont(workbook);
